...
public class UserServiceImpl implements UserService {
    ...
    public boolean insertUser(User user) {
        ...
        if (boo) {
            logger.info("添加用户成功：" + user.getPhoneNumber());
            logger.info("开始添加权限：权限ID为3");
            boolean b = roleDao.insertRoleId_userId(Long.parseLong(user.getPhoneNumber()), 3);
            if (b)
                logger.info("权限添加成功："+b);
            else
                logger.info("权限添加失败："+b);
        } else {
            logger.info("添加用户失败：" + user.getPhoneNumber());
        }
        return boo;
    }
